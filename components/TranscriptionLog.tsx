...
const TranscriptionLog: React.FC<TranscriptionLogProps> = ({ entries, liveTranscript, emptyMessage = "Start listening to see transcription here..." }) => {
    ...
    return (
        <div className="w-full h-40 md:h-48 bg-gray-900/50 rounded-lg p-4 overflow-y-auto border border-gray-700 shadow-inner">
            {entries.length === 0 && !liveTranscript ? (
                ...
                <>
                    {entries.map((entry) => (
                        <div key={entry.id} className={`mb-3 p-3 rounded-md bg-gray-800 border-l-4 ${emotionColorMapping[entry.emotionState?.emotion || Emotion.Neutral]}`}>
                    ...
                    {liveTranscript && (
                        <div className="mb-3 p-3 rounded-md bg-gray-800 border-l-4 border-blue-500 opacity-75">
                            <p className="text-gray-300">{liveTranscript}<span className="inline-block w-1 h-4 bg-yellow-400 ml-1 animate-pulse"></span></p>
                            <p className="text-xs text-gray-500 mt-1 text-right">
                                Listening...
                            </p>
                        </div>
